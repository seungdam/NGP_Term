...
int SOCKETINFO::ServerDoSendMovePacket(int id)
{
	S2C_PLAYER_MOVE_PACKET mp;
	mp.type = (char)SERVER_PACKET_INFO::PLAYER_MOVE;
	mp.p_dir = m_PlayersInfo[id].p_dir;
	mp.p_id = id;
	int retval = send(m_sock, (char*)&mp, sizeof(S2C_PLAYER_MOVE_PACKET), 0);
	return retval;
}
...
void SOCKETINFO::ProcessPacket(char* data)
{
	...
	case CLIENT_PACKET_INFO::MOVE:
	{
		...
		m_PlayersInfo[info->from_c_id].p_id = info->from_c_id;
		m_PlayersInfo[info->from_c_id].p_dir = info->direction;
